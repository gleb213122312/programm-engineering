...
В лабораторных я последовательно закрепил базовые приёмы ООП. Я научился объявлять свои классы и создавать объекты, добавлять атрибуты и методы, переопределять поведение через наследование, ограничивать доступ к данным инкапсуляцией и вызывать одинаковые методы у разных типов через полиморфизм. По ходу работ я проверял результат через понятные выводы в консоль — это помогало видеть, что объект создаётся, поля заполняются корректно, методы считают нужные значения, а переопределение и инкапсуляция работают как задумано. В итоге я лучше понимаю, как строить код, который можно расширять без ломки уже написанного.

Итог по самостоятельным заданиям про книги 

В самостоятельной части я применил те же принципы на теме “книги”:

Я создал свой класс и объект, чтобы закрепить базовую структуру.

Я добавил реальные атрибуты и методы (short_info, age), чтобы увидеть, как объект хранит состояние и выполняет вычисления.

Я сделал наследника с рейтингом и переопределил метод, потренировав super() и расширение функциональности без изменения базового класса.

Я использовал инкапсуляцию для цены, ограничил некорректные значения через метод установки и убедился, что приватное поле не доступно напрямую.

Я реализовал полиморфизм: электронная и бумажная книга имеют общий интерфейс describe, поэтому их можно обрабатывать одним циклом/функцией.

В целом эти задания показали мне, как ООП упрощает поддержку и развитие программы: код становится переиспользуемым, понятным по ролям (данные/поведение) и легко расширяемым новыми типами без переписывания остальной логики.
